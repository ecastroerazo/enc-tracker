 ...
 *
 * This module handles all logic for displaying, filtering, exporting, adding, editing, and deleting withdrawal records for the ENC Tracker system.
 *
 * Main features:
 *  - Render report filters for withdrawals
 *  - Export withdrawals as CSV
 *  - Display withdrawals table with summary
 *  - Handle add, edit, and delete withdrawal actions
 *  - Provide shortcodes for withdrawal forms and editing
 */
...
/**
 * Handles exporting withdrawal records as CSV if the correct GET param is set.
...
// =============================
// Main Withdrawals View
// =============================

/**
 * Main withdrawals view: displays the withdrawal table, summary, filter, and handles view switching.
 * Handles:
 *  - Edit mode (delegates to edit shortcode)
 *  - Success/error messages
 *  - Filtering and summary
 *  - Table rendering
 *  - Export and add withdrawal links
 *
 */
function enc_withdrawals_view()
{
    ...
    // Build SQL WHERE clause for filtering withdrawals
    $store_id = intval($_GET['store_id'] ?? 0);
    $date_from = enc_parse_date($_GET['date_from'] ?? '', current_time('Y-m-01'));
    $date_to = enc_parse_date($_GET['date_to'] ?? '', current_time('Y-m-d'));
    ...
    // Main query for withdrawals (with store join)
    $base_withdrawal_query = "SELECT w.*, s.name as store_name FROM {$wpdb->prefix}enc_withdrawals w 
            JOIN {$wpdb->prefix}enc_stores s ON w.store_id = s.id 
            $where ORDER BY w.withdrawal_date DESC, w.id DESC";

    // Fetch withdrawals and calculate summary stats
...
// =============================
// Withdrawal Form Handling (Add/Edit/Delete)
// =============================

/**
 * Handles POST submissions for adding, editing, and deleting withdrawals.
...
function enc_handle_withdrawal_forms()
{
    ...
    // Handle delete withdrawal action
...
// =============================
// Shortcodes for Withdrawal Edit and Submission
// =============================

/**
 * Shortcode: [enc_withdrawal_edit]
 * Renders the edit form for a specific withdrawal record (admin only).
 */
    // ...existing code...
/**
 * Shortcode: [enc_withdrawal_form]
 * Renders the form for submitting a new withdrawal record (for users with permission).
 */
